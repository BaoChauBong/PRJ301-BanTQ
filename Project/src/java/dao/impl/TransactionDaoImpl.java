...
public class TransactionDaoImpl extends connectDB implements TransactionDao {
    ...
    public void insert(Transactions transaction) {
        String sql = "INSERT INTO transactions(user_session,user_name,user_mail,user_phone,"
                + "address,message,amount,payment,created) VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    public void edit(Transactions transaction) {
        String sql = "Update transactions set user_name =?, user_mail =?, user_phone =?, address= ?,"
                + "message=?,amount=?,payment=?, status=? where id=?";
        Connection con = connectDB.getConnect();
        try {
            PreparedStatement ps = con.prepareStatement(sql);
